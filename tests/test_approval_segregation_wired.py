...
@pytest.mark.xfail(strict=True, reason=(
    "NOT WIRED, AND NOT WIRED ON PURPOSE THREE HOURS BEFORE A DEADLINE. "
    "The call was written into review.py::resolve_escalation and reverted the "
    "same hour. It worked -- and it turned five screen tests red and would have "
    "turned the live demonstration red with them, because production runs "
    "SEGREGATED (STRATA_APPROVAL_MODE is unset, and unset means the safe value) "
    "while the demonstration has one person playing every role. Gate 4 refused "
    "them, correctly. Making it pass means either setting DEMO_SELF_APPROVAL on "
    "the deployment so the waiver is audited rather than the control skipped, or "
    "giving the seed a second person to review the first one's work. Both are "
    "right; neither is a change to who may approve what that should ship "
    "untested at this hour. STRICT, so wiring it fails this test and forces its "
    "own removal."
))
