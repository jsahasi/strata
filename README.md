...
| | lines |
|---|---:|
| Application — `app/`, 69 Python modules | 41,771 |
| Tests — `tests/` | 30,295 |
| Templates — Jinja | 5,355 |
| Front end — hand-written CSS and JS, no build step | 6,400 |
| Scripts — seeding, migration, status, backup | 1,808 |
| Documents — `docs/`, HTML | 7,576 |
| Marketing site — `deploy/site/` | 5,894 |
| **Total** | **99,099** |

Tests are 72 per cent of the application by line. No test count is written here for the
...
- **Nothing fetches on a schedule.** The source registry records where filings come from and
  says plainly that no fetcher exists. `ScheduledRun` is a table with cadences and nothing
  polling.
- **Retention is written and not running.** The schedule covers every table with a window and a
  reason, the purges are tested, and nothing calls them.
- **No rollback to a point in time.** A reversal is a new audit row pointing at the row it
  reverses; replaying state to an earlier moment is not built.
